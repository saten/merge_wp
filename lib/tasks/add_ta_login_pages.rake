    ...
    DataMapper.repository(:destination) do      
      Wp::Blog.all.each do |blog|
        ...
        begin
          ...
          "(#{last_id+1},1,'#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}','#{Time.now.gmtime.strftime("%Y-%m-%d %H:%M:%S")}','','TeamArtist Login','','publish','closed','closed','ta_login','','','#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}','#{Time.now.gmtime.strftime("%Y-%m-%d %H:%M:%S")}','','http://#{domain}/?page_id=#{last_id+1}','page'),"\
          "(#{last_id+2},1,'#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}','#{Time.now.gmtime.strftime("%Y-%m-%d %H:%M:%S")}','','Ottieni una nuova password','','publish','closed','closed','','','ta_forgot_password','#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}','#{Time.now.gmtime.strftime("%Y-%m-%d %H:%M:%S")}','','http://#{domain}/?page_id=#{last_id+2}','page'),"\
          "(#{last_id+3},1,'#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}','#{Time.now.gmtime.strftime("%Y-%m-%d %H:%M:%S")}','','Resetta password','','publish','closed','closed','reset_password','','','#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}','#{Time.now.gmtime.strftime("%Y-%m-%d %H:%M:%S")}','','http://#{domain}/?page_id=#{last_id+3}','page');")
